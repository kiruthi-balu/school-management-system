...
def create_academic_year_student(
    ac_class_id: int = Form(...),
    student_id: int = Form(...),
    user: User = Depends(master_access_user),
    db: Session = Depends(get_db)
):
    existing = db.query(AcademicYearStudent).filter(
        AcademicYearStudent.ac_class_id == ac_class_id,
        AcademicYearStudent.student_id == student_id,
        AcademicYearStudent.status != -1
    ).first()
