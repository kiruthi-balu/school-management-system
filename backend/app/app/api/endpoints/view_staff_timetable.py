...
@router.post("/view-timetable-by-staff/")
def view_timetable_by_staff(staff_id: int = Query(..., description="Staff User ID"),user: User = Depends(master_access_user),db: Session = Depends(get_db)):
    
    allocations = db.query(SubjectAllocation).filter(SubjectAllocation.user_id == staff_id,SubjectAllocation.status != -1).all()

    if not allocations:
        return{"Status":0, "Message":"No subject allocations found for this staff"}
