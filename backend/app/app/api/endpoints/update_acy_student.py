from fastapi import APIRouter, Depends, Form, Path
from sqlalchemy.orm import Session
from app.models import AcademicYearStudent, User
from app.api.deps import get_db, admin_only_user

router = APIRouter()

@router.post("/update-academic-year-student/{student_record_id}")
def update_academic_year_student(
    student_record_id: int = Path(...),
    student_id: int = Form(...),
    ac_class_id: int = Form(...),
    user: User = Depends(admin_only_user),
    db: Session = Depends(get_db)
):
    record = db.query(AcademicYearStudent).filter(AcademicYearStudent.id == student_record_id, AcademicYearStudent.status != -1).first()

    if not record:
        return{"Status":0,"Message":"Academic year student record not found"}

    record.student_id = student_id
    record.ac_class_id = ac_class_id
    record.m_by = user.id

    db.commit()
    db.refresh(record)

    return {"status": 1, "message": "Academic year student record updated", "id": record.id}
